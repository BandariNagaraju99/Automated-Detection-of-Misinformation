...
from analyzer   import (
    ...
    temporal_label, EVENT_VERB_GROUPS,
)
...
DEBUNK_SOURCES = {
    'factly', 'snopes', 'politifact', 'factcheck', 'alt news', 'altnews', 
    'boomlive', 'fact crescendo', 'vishvas news', 'webqoof', 'poynter', 'quint'
}

DEBUNK_KEYWORDS = {
    'fact check', 'fact-check', 'debunked', 'debunk', 'false', 'misleading', 
    'fake', 'hoax', 'myth', 'untrue', 'not true', 'not the cause', 'does not cause',
    'is not true', 'lacks evidence', 'incorrect', 'rumor', 'rumour', 'fake news'
}
...
class TruthLensPredictor:
    ...
    def __init__(
        ...
        self._cross_encoder = CrossEncoder('cross-encoder/ms-marco-MiniLM-L-6-v2', device=self._device)

        # 2. ANN model
        self._ann = FakeNewsANN(input_dim=768).to(self._device)
        self._load_ann(model_path)
        self._ann.eval()

        # 3. RAG engine (shares sentence encoder and cross-encoder)
        self.rag = RAGEngine(self._encoder, self._cross_encoder)
    ...
    def predict(self, query: str, top_k: int = 8) -> PredictionResult:
        ...
        # Strip auto-appended Telugu characters for the English ANN model
        q_ann = q_clean
        import re
        if not re.search(r'[\u0c00-\u0c7f]', query):
            q_ann = re.sub(r'[\u0c00-\u0c7f]+', '', q_ann).strip()
            q_ann = re.sub(r'\s+', ' ', q_ann)
        ...
        rag_results, avg_sim = self.rag.search(q_clean, top_k=top_k, temporality=temporality)
        ...
        if force_live:
            # Clean query for Google News: if original query had no Telugu script, strip Telugu script from q_clean
            gnews_query = q_clean
            import re
            if not re.search(r'[\u0c00-\u0c7f]', query):
                # Remove Telugu characters (range \u0c00-\u0c7f)
                gnews_query = re.sub(r'[\u0c00-\u0c7f]+', '', gnews_query).strip()
                # Clean double spaces
                gnews_query = re.sub(r'\s+', ' ', gnews_query)

            live_hits = self.rag.search_live(gnews_query, top_k=8)
            if live_hits:
        ...
        # ── 5. Consensus detection & Matched Debunk Detection ──
        consensus = any(r.match_type == _CONSENSUS_MATCH_TYPE for r in rag_results)

        is_debunked_claim = False
        debunking_article = None
        for r in rag_results[:3]:
            title_lower = r.article.title.lower()
            source_lower = r.article.source.lower()
            if r.similarity > 0.60:
                matched_debunk_source = any(s in source_lower for s in DEBUNK_SOURCES)
                matched_debunk_title = any(kw in title_lower for kw in DEBUNK_KEYWORDS)
                if matched_debunk_source or matched_debunk_title:
                    is_debunked_claim = True
                    debunking_article = r.article
                    break

        # ── 6. Decision formula (Dynamic Contextual Weighting) ──
        # Check if the query asserts a major event and lacks corroborating news
        q_words = clean_text(query).split()
        has_event_verb = any(w in EVENT_VERB_GROUPS[group] for group in EVENT_VERB_GROUPS for w in q_words)
        is_event_unsupported = False
        if has_event_verb and max_sim < 0.50:
            is_event_unsupported = True

        # If we have a strong news match, trust RAG completely and ignore the ANN's stylistic score.
        # Otherwise, scale between RAG and ANN.
        if max_sim >= 0.70:
            w_rag = 1.0
            w_ann = 0.0
        elif max_sim >= 0.45:
            # Linear interpolation between 0.70 (1.0 RAG) and 0.45 (0.40 RAG)
            w_rag = 0.40 + 0.60 * ((max_sim - 0.45) / 0.25)
            w_ann = 1.0 - w_rag
        else:
            w_rag = 0.40
            w_ann = 0.60

        # Don't let low average similarity drag down a single extremely strong match (e.g. regional news)
        effective_rag_sim = max(0.6 * max_sim + 0.4 * avg_sim, max_sim * 0.90)

        final_real = w_rag * effective_rag_sim + w_ann * (1.0 - ann_fake)
        ...
        if is_debunked_claim:
            final_real = 0.10                        # matched a debunking article → FAKE

        if is_event_unsupported:
            final_real = min(final_real, 0.25)       # unsupported major event claim → FAKE/LIKELY FAKE
        ...
        # ── 7. Build explanation ──────────────────────────────
        reasons: list[str] = []
        has_claim_mismatches = any(r.match_type == 'CLAIM_MISMATCH' for r in rag_results)
        ...
        if is_debunked_claim and debunking_article:
            reasons.append(f"❌ Debunked by {debunking_article.source}: {debunking_article.title}")
        elif is_event_unsupported:
            reasons.append('❌ Factual mismatch: No trusted news reports verify this event claim')
        elif consensus:
        ...
        if has_claim_mismatches:
            reasons.append('⚠️ Search results mention terms but report different events/contexts')
        ...
        articles_out = [
            ...
            for r in rag_results if r.similarity >= 0.20
        ]

        # Group and sort by age: Today (<24h), Yesterday (<48h), and Older (>=48h)
        today_list = []
        yesterday_list = []
        older_list = []
        for art in articles_out:
            age = art.get('age_hours', 999.0)
            if age < 24:
                today_list.append(art)
            elif age < 48:
                yesterday_list.append(art)
            else:
                older_list.append(art)
        
        # Sort each subgroup by similarity descending
        today_list.sort(key=lambda x: x['similarity'], reverse=True)
        yesterday_list.sort(key=lambda x: x['similarity'], reverse=True)
        older_list.sort(key=lambda x: x['similarity'], reverse=True)
        
        articles_out = today_list + yesterday_list + older_list

        ms = (time.time() - t0) * 1000
        log.info(
            "query='%.50s'  topic=%s  loc=%s  temporal=%s  ann=%.3f  sim=%.3f  "
            'real=%.3f  verdict=%s  %.0fms',
            query, topic, q_location, temporality, ann_fake, effective_rag_sim,
        ...
        return PredictionResult(
            ...
            rag_similarity   = effective_rag_sim,
            # sensationalism   = sens,
            # attribution      = attr,
            topic            = topic,
            location         = q_location,
