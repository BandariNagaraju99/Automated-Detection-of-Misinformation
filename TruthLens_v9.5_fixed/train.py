...
def main():
    ...
    log.info('Device: %s (multi-threaded CPU execution enabled)', device)
    ...
    # Clean only the sampled records to optimize speed
    balanced['content'] = (balanced['title'].fillna('') + ' ' + balanced['text'].fillna('')).apply(clean)
    balanced = balanced[balanced['content'].str.len() > 15]
